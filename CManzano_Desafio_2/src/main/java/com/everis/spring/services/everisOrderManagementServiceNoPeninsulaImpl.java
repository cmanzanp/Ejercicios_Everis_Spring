...
public class everisOrderManagementServiceNoPeninsulaImpl implements everisOrderManagementServiceI {
	...
	public EverisOrder createOrder(final String everisOrderId, final String deliveryAddress, final String addressee) {
		...
		everisOrder.setExceptionalAddress(true);
	...
	public void addProduct(EverisOrder order, EverisProduct product) {
		...
		product.setProductPricePVP(product.getProductPriceNoTaxes()*1.04);
	...
	public void showOrder(EverisOrder order) {
		...
		System.out.println("Pedido para fuera de la península");
